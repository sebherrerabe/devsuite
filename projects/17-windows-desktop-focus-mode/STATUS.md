...
- [x] Hardened website blocking to treat active desktop sessions (`RUNNING` + `PAUSED`) as hosts-enforced when blocked domains exist, with explicit degraded-state surfacing instead of silent failure (2026-03-11)
...
| 2026-02-14 | Website blocking uses hosts-based hard blocking during active sessions, while URL telemetry remains signal-aware for warn/escalate reminders only; never force-close browsers from partial URL context.      | Preserves safety around unreliable browser telemetry while still enforcing blocked domains for active sessions.    | User + Codex |
| 2026-03-11 | Hosts enforcement health is explicit (`active`/`degraded`/`inactive`) and degraded helper paths must notify/audit instead of silently continuing.                                                            | Makes packaged Windows enforcement failures actionable and observable for users and QA.                            | User + Codex |
