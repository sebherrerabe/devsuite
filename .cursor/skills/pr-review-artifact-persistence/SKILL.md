...
const submitPRReviewSchema = z.object({
  ...
  metadata: z
    .record(z.any())
    .optional()
    .describe('Optional structured metadata (signals, risk areas, etc.)'),
  sessionId: z
    .string()
    .optional()
    .describe('Optional session ID to link review to work session'),
  taskId: z
    .string()
    .optional()
    .describe('Optional task ID to link review to task'),
});
```
